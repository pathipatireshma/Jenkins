...
cat <<EOF >/tmp/jobs
Terraform,VPC,https://github.com/pathipatireshma/terraform-vpc,123456780,yes
Terraform,DB,https://github.com/pathipatireshma/terraform-databases,123456781,yes
Terraform,ALB,https://github.com/pathipatireshma/terraform-mutable-alb,123456779,yes
Terraform,Muable-Ec2-Module,https://github.com/pathipatireshma/terraform-mutable-ec2.git,123456782,yes
Terraform,Cart,https://github.com/pathipatireshma/cart.git,123456782,yes,mutable/Jenkinsfile
CI,cart,https://github.com/pathipatireshma/cart.git,123456784,yes
CI,catalogue,https://github.com/pathipatireshma/catalogue.git,123456785,yes
CI,user,https://github.com/pathipatireshma/user.git,123456786,yes
CI,shipping,https://github.com/pathipatireshma/shipping.git,123456787,yes
CI,payment,https://github.com/pathipatireshma/payment.git,123456788,yes
CI,frontend,https://github.com/pathipatireshma/frontend.git,123456788,yes
EOF
...
EOF
  ...
  cat /tmp/folder.xml | java -jar ~/jenkins-cli.jar -auth admin:admin -s http://172.31.7.219:8080/ -webSocket create-job ${DIR}
  cat /tmp/job.xml | java -jar ~/jenkins-cli.jar -auth admin:admin -s http://172.31.7.219:8080/ -webSocket create-job ${JOB_NAME}
  if [ "$(echo $job | awk -F , '{print $5}')" == "yes" ]; then

    GIT_ORG_REPO_NAME=$(echo $GIT_URL | awk -F / '{print $5}' | sed 's/.git$//')
curl "https://api.github.com/repos/pathipatireshma/${GIT_ORG_REPO_NAME}/hooks" \
     ...
     -d @- << EOF
{
  ...
  "config": {
    "url": "http://44.203.220.118:8080/multibranch-webhook-trigger/invoke?token=${JOB_ID}",
    "content_type": "json"
  }
}
EOF
